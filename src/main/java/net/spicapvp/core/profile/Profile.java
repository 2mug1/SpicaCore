...
public class Profile {
	...
	public void findAlternates() {
		if (this.currentAddress != null) {
			try (MongoCursor<Document> cursor = SpicaCore.get().getMongo().getProfiles().find(Filters.eq("currentAddress", this.currentAddress)).iterator()) {
	...
	public void setupBukkitPlayer(Player player) {
		...
		PermissionAttachment attachment = player.addAttachment(SpicaCore.get());
	...
	public void load() {
		Document document = SpicaCore.get().getMongo().getProfiles().find(Filters.eq("uuid", uuid.toString())).first();
		...
		if (document != null) {
			...
			ipAddresses = SpicaCore.GSON.fromJson(document.getString("ipAddresses"), SpicaCore.LIST_STRING_TYPE);
	...
	public void save() {
		...
		document.put("ipAddresses", SpicaCore.GSON.toJson(ipAddresses, SpicaCore.LIST_STRING_TYPE));
	...
	public static Profile getByUsername(String username) {
		...
		UUID uuid = SpicaCore.get().getRedisCache().getUuid(username);
	...
	public static List<Profile> getByIpAddress(String ipAddress) {
		...
		try (MongoCursor<Document> cursor = SpicaCore.get().getMongo().getProfiles().find(filter).iterator()) {
