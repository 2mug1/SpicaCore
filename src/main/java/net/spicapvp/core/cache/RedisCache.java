package net.spicapvp.core.cache;
...
public class RedisCache extends Strapped {
	...
	public RedisCache(SpicaCore spicaCore) {
		super(spicaCore);
	}

	public UUID getUuid(String name) {
		if (spicaCore.getServer().isPrimaryThread()) {
			throw new IllegalStateException("Cannot query on main thread (Redis profile cache)");
		}

		try (Jedis jedis = spicaCore.getJedisPool().getResource()) {
			...
			spicaCore.debug(Level.WARNING, "Could not connect to redis", e);
		}
		...
		try {
			...
			spicaCore.debug(Level.WARNING, "Could not fetch from Mojang API", e);
		}
