...
public class Rank {
	...
	public void load() {
		load(SpicaCore.get().getMongo().getRanks().find(Filters.eq("uuid", uuid.toString())).first());
	}
	...
	private void load(Document document) {
		...
		SpicaCore.GSON.<List<String>>fromJson(document.getString("inherits"), SpicaCore.LIST_STRING_TYPE)
				.stream()
	...
	public void save() {
		...
		document.put("inherits", SpicaCore.GSON.toJson(inherited
				.stream()
				.map(Rank::getUuid)
				.map(UUID::toString)
				.collect(Collectors.toList())));

		SpicaCore.get().getMongo().getRanks().replaceOne(Filters.eq("uuid", uuid.toString()), document,
				new ReplaceOptions().upsert(true));

		SpicaCore.get().getPidgin().sendPacket(new PacketRefreshRank(uuid, displayName));
	}

	public void delete() {
		ranks.remove(uuid);
		SpicaCore.get().getMongo().getRanks().deleteOne(Filters.eq("uuid", uuid.toString()));

		SpicaCore.get().getPidgin().sendPacket(new PacketDeleteRank(uuid));
	}

	public static void init() {
		Map<Rank, List<UUID>> inheritanceReferences = new HashMap<>();

		try (MongoCursor<Document> cursor = SpicaCore.get().getMongo().getRanks().find().iterator()) {
			while (cursor.hasNext()) {
				...
				SpicaCore.GSON.<List<String>>fromJson(document.getString("permissions"), SpicaCore.LIST_STRING_TYPE)
